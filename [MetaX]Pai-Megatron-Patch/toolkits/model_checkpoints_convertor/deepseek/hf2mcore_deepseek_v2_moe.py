...
def load_megatron_model(args):
    ...
    if (
        ...
        if not args.moe_batched_gemm:
            ...
            for k in w1:
                w1[k] = torch.cat(w1[k], dim=0)
            for k in w2:
                w2[k] = torch.cat(w2[k], dim=0)

            state_dict.update(w1)
            state_dict.update(w2)
            ...
            if layer_idx == 0:
                ...
                if not args.moe_batched_gemm:
                    ...
                    for expert_id, hfexpert in enumerate(hflayer.mlp.experts):
                        gate_weight, up_weight = torch.chunk(mglayer.mlp.experts.weight1[expert_id,:,:].permute(1,0), chunks=2, dim=0)
                        hfexpert.gate_proj.weight.copy_(gate_weight)
                        hfexpert.up_proj.weight.copy_(up_weight)
                        hfexpert.down_proj.weight.copy_(mglayer.mlp.experts.weight2[expert_id,:,:].permute(1,0))
        ...
        for layer_idx, (mglayer, hflayer) in enumerate(zip(mgmodel.decoder.layers, hfmodel.model.layers)):
            ...
            if layer_idx == 0:
                ...
                if not args.moe_batched_gemm:
                    ...
                    mglayer.mlp.router.weight.copy_(hflayer.mlp.gate.weight)
                    fc1_weights_allexpert = []
                    fc2_weights_allexpert = []
                    for hf_expert in hflayer.mlp.experts:
                        fc1_weight_local_epert = torch.cat([hf_expert.gate_proj.weight, hf_expert.up_proj.weight])
                        fc2_weight_local_epert = hf_expert.down_proj.weight
                        fc1_weights_allexpert.append(fc1_weight_local_epert)
                        fc2_weights_allexpert.append(fc2_weight_local_epert)
                    fc1_weights_batched = torch.stack(fc1_weights_allexpert, dim=0)
                    fc2_weights_batched = torch.stack(fc2_weights_allexpert, dim=1)
                    fc1_weights_batched_transposed = fc1_weights_batched.permute(0, 2, 1)
                    fc2_weights_batched_transposed = fc2_weights_batched.permute(1, 2, 0)
                    
                    mglayer.mlp.experts.weight1.copy_(fc1_weights_batched_transposed)
                    mglayer.mlp.experts.weight2.copy_(fc2_weights_batched_transposed)
    ...
    if (
        ...
        for ep_rank in range(args.expert_model_parallel_size):
            ...
            for k, v in full_model.items():
                # expert=64, ep=8:  rank0:[expert0~expert7]  rank1:[expert8~expert15]
                if not args.moe_batched_gemm:
                    if 'local_experts' in k:
                    ...
                    if 'mlp.experts.weight1' in k:
                        v = v[args.expert_model_parallel_size * ep_rank : args.expert_model_parallel_size * (ep_rank + 1) , :, :].contiguous()
                    elif 'mlp.experts.weight2' in k:
                        v = v[args.expert_model_parallel_size * ep_rank : args.expert_model_parallel_size * (ep_rank + 1) , :, :].contiguous()
                    model_split[k] = v
