...
class GroupedMLP(MegatronModule):
    ...
    def __init__(self, num_local_experts: int, config: TransformerConfig):
        try:
            import apex.grouped_gemm
        except ImportError as e:
            raise ImportError("Required `apex` with grouped_gemm is not installed!") from e
    ...
    def forward(self, permuted_local_hidden_states, tokens_per_expert):
        if permuted_local_hidden_states.nelement() != 0:
            ...
            fc1_output = apex.grouped_gemm.gmm(
                permuted_local_hidden_states, w1, tokens_per_expert, trans_b=False
            )

            intermediate_parallel = self.activation_func(fc1_output)

            fc2_output = apex.grouped_gemm.gmm(intermediate_parallel, w2, tokens_per_expert, trans_b=False)
        else:
...
class SequentialMLP(MegatronModule):
    ...
    def forward(self, permuted_local_hidden_states, tokens_per_expert):
        ...
        zero_tensor = torch.empty(1, dtype=torch.long, device=cumsum_num_tokens.device)
...
class BatchedMLP(MegatronModule):
    """An implementation of the Experts layer using a batched gemm  of MLP layers.
    use batched gemm when paddding
    """
    ...
    def __init__(self, num_local_experts, config: TransformerConfig):
        ...
        if self.config.gated_linear_unit:
            if self.config.activation_func not in (F.silu, F.gelu):
                raise ValueError("Activation function must be silu or gelu when using BatchedMLP.")
    ...
    def forward(self, permuted_local_hidden_states: torch.Tensor, tokens_per_expert: torch.Tensor):
        """Forward step of the BatchedMLP."""
        seq_len_for_all_expert, hidden_size = permuted_local_hidden_states.shape
        expert_number = len(tokens_per_expert)
        if seq_len_for_all_expert % expert_number == 0:
            same_tokens_per_expert = seq_len_for_all_expert // expert_number
            permuted_local_hidden_states_batched = permuted_local_hidden_states.view(expert_number, same_tokens_per_expert, hidden_size)  # 8 256 512
        else:
            raise ValueError(" must be divided evenly by expert_number for batched gemm") 
        
        # Reshape the weights for the batached GEMMs.

        # w1 = self.weight1.transpose(0,1).view(self.num_local_experts, -1, self.config.hidden_size).permute(0,2,1)
        # w2 = self.weight2.view(self.num_local_experts, -1, self.config.hidden_size)
        # print('permuted_local_hidden_states_batched', permuted_local_hidden_states_batched.shape)
        fc1_output = torch.matmul(permuted_local_hidden_states_batched, self.weight1)
        intermediate_parallel = self.activation_func(fc1_output)
        fc2_output = torch.matmul(intermediate_parallel, self.weight2)
        # print('fc2_output', fc2_output.shape)
        fc2_output = fc2_output.view(seq_len_for_all_expert, hidden_size)
        # print('permuted_local_hidden_states_batched', permuted_local_hidden_states_batched.shape)
