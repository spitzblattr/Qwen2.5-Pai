...
# LOAD_CHECKPOINT_PATH=/mnt/public/model/huggingface/Qwen2.5-7B-Instruct   # 原始 huggingface模型路径
# SAVE_CHECKPOINT_PATH=/mnt/public/model/spitzblattr/training_models/Qwen2.5-7B-Instruct  # SAVE_CHECKPOINT_PATH 下必须是空白的

# >>>>>>> 以下是当 CONVERT_TYPE=megatron-to-huggingface 时的路径设置，否则注释掉以下
HF_MODEL_PATH=/mnt/public/model/huggingface/Qwen2.5-7B-Instruct  # 原始 huggingface模型路径
LOAD_CHECKPOINT_PATH=/mnt/public/model/spitzblattr/training_models/saved_ckpts  # 训练完成的模型
SAVE_CHECKPOINT_PATH=/mnt/public/model/spitzblattr/train_finished_models/final   # SAVE_CHECKPOINT_PATH 下必须是空白的
# --------------------------------------------------------
...
additional_args=(
    ...
    #--target-num-layers-per-virtual-pipeline-stage 1   # 沐曦版Megatron未实现
    ...
    --save-safetensors
    --bf16
    --transformer-impl local  # 沐曦目前不支持 transformer_engine
)


torchrun ${distributed_args[@]} /workspace/Pai-Megatron-Patch/toolkits/model_checkpoints_convertor/qwen/hf2mcore_qwen2_dense_and_moe_gqa.py \
    ${model_config_args[@]} \
    ${additional_args[@]} \
    ${convert_args[@]} \
